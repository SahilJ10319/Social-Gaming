...
private:
  ...
  bool messageContainsCommand(const std::string& message);
...
bool Channel::messageContainsCommand(const std::string& message)
{
  char commandIdentifier = '/';

  return message[0] == commandIdentifier;
