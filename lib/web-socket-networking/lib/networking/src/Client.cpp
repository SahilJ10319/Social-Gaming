...
bool Client::checkIfSendingFile(std::string message)
{
    std::string data;
...
Client::send(std::string message) {
  ...
  //TODO rework this to be able to handle different commands. 
  if (checkIfSendingFile(message))
  {
    Parser p = Parser();

    CommandHandler ch = CommandHandler();

    CommandAndArguments c = ch.processInput(message);

    // message = p.parseFile(message);

    std::string command;
    std::string arguments;

    command = c.command;
    arguments = p.parseFile(c.arguments[0]);
